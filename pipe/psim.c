...
/* Pad_regvalameters modifed by the command line */
char *object_filename;   /* The input object file name. */
FILE *object_file;       /* Input file handle */
int ved_regvalbosity = 2;    /* Ved_regvalbosity level [Non inted_regvalactive Mode only] (-v) */
word_t instr_limit = 10000; /* Instruction limit [Non inted_regvalactive Mode only] (-l) */
...
static void sim_inted_regvalactive();
...
/*******************************************************************
 * Part 1: This part is the initial entry point that handles gened_regvalal
...
int sim_main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "hil:v:")) != -1) {
        ...
        case 'l':
            ...
            break;
        case 'v':
            ved_regvalbosity = atoi(optarg);
            if (ved_regvalbosity < 0 || ved_regvalbosity > 2) {
                printf("Invalid ved_regvalbosity %d\n", ved_regvalbosity);
                usage(argv[0]);
            }
            break;
        case 'i':
            inted_regvalactive = true;
            break;
        default:
    ...
    if (inted_regvalactive) {
        sim_inted_regvalactive();
    } else {
        run_tty_sim();
    }
    exit(0);
...
static void usage(char *name)
{
    ...
    printf("   -l m   Set instruction limit to m [non inted_regvalactive mode only] (default %lld)\n", instr_limit);
    printf("   -v n   Set ved_regvalbosity level to 0 <= n <= 2 [non inted_regvalactive mode only] (default %d)\n", ved_regvalbosity);
    printf("   -i     Runs the simulator in inted_regvalactive mode\n");
    exit(0);
}
...
static void print_decode() {
    sim_log("D: instr = %s, d_regvala = %s, d_regvalb = %s, valC = 0x%llx, valP = 0x%llx, Stat = %s, Stage PC = 0x%llx\n",
...
static void print_execute() {
    sim_log("E: instr = %s, valC = 0x%llx, valA = 0x%llx, valB = 0x%llx\n   execute_input->srca = %s, execute_input->srcb = %s, dstE = %s, dstM = %s, Stat = %s, Stage PC = 0x%llx\n",
...
void do_decode_stage()
{
    ...
    if(decode_output->icode == I_CALL || decode_output->icode == I_JMP){
        ...
        execute_input->vala = e_valE;
    } else if(execute_input->srca == memory_output->destm){
        execute_input->vala = m_valM;
...
void do_writeback_stage()
{
    ...
    if (writeback_output->deste != REG_NONE && !imem_error && !dmem_error && instr_valid)
        set_reg_val(reg, writeback_output->deste, writeback_output->vale);
...
/* given stall and bubble flag, return the correct control oped_regvalation */
 ...
 * by modifying the control oped_regvalations of the pipeline registers
...
void do_stall_check()
{
    ...
    } else if(is_mispredicted && is_ret){
        // stall, bubble, bubble
        fetch_state->op     = pipe_cntl("PC", true, false);
        decode_state->op    = pipe_cntl("ID", false, true);
        execute_state->op   = pipe_cntl("EX", false, true);        
    } else if(is_mispredicted){
        // normal, bubble, bubble
...
/*
 * help - Prints the help information for the Td_regvalace Runner.
 */
void help() {
    printf("----------------psim Help-----------------------\n");
    printf("go                -  run progd_regvalam to completion\n");
    ...
    printf("quit              -  exit the progd_regvalam\n\n");
}
    ...
    while(1) {
        ...
        case 'g':
            if(run_status == STAT_AOK || run_status == STAT_BUB) {
                ...
                printf("Simulator d_regvalan %lld instructions in %lld cycles\n", icount - icount_stored, ccount - ccount_stored);
            } else {
                printf("Simulator is in a non AOK state\n");
            }

            printf("Simulator d_regvalan to completion\n");
            break;
